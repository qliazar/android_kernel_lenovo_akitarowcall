...
#endif
#define HI556TXD_OTP_FUNCTION 1
#if HI556TXD_OTP_FUNCTION
#define HI556TXD_LSC_DATA_SIZE 1868
#define HI556TXD_AWB_DATA_SIZE 16
extern unsigned char hi556txd_data_af[MODULE_AF_DATA_SIZE + 1];
extern unsigned char hi556txd_data_lsc[HI556TXD_LSC_DATA_SIZE + 1];
extern unsigned char hi556txd_data_awb[HI556TXD_AWB_DATA_SIZE + 1];
extern unsigned char hi556txd_module_id;
extern unsigned char hi556txd_lsc_valid;
extern unsigned char hi556txd_awb_valid;
extern unsigned char hi556txd_af_valid;
#endif

#define SC500CS_OTP  1

#if SC500CS_OTP
#define SC500CS_OTP_SIZE 0x077E + 1
extern unsigned char sc500cs_otp_data[SC500CS_OTP_SIZE];
//AWB 51K
#define SC500CS_AWB_51K_INFO_START 0x18
#define SC500CS_AWB_51K_DATA_SIZE 16
extern unsigned char sc500cs_awb_51k_data_valid;
//LSC 51K
#define SC500CS_LSC_51K_INFO_START 0x31
#define SC500CS_LSC_51K_DATA_SIZE 1868
extern unsigned char sc500cs_lsc_51k_data_valid;
//AF
#define SC500CS_AF_MACRO_START 0x2b
#define SC500CS_AF_MACRO_DATA_SIZE 2

#define SC500CS_AF_INF_START 0x2d
#define SC500CS_AF_INF_DATA_SIZE 2
extern unsigned char sc500cs_af_51k_data_valid;
#endif
...
#endif
{
...
#endif
#if HI556TXD_OTP_FUNCTION
    if (ptempbuf->sensorID == 0x557 && ptempbuf->u4Length == 0x1 && ptempbuf->u4Offset == 0x402){//HI846 MID data
      if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)&hi556txd_module_id, ptempbuf->u4Length)) {
        kfree(pBuff);
        kfree(pu1Params);
        pr_debug("ioctl copy to user failed\n");
        return -EFAULT;
    ...
    if (ptempbuf->sensorID == 0x557 && ptempbuf->u4Length == 0x74C && ptempbuf->u4Offset == 0x48a && hi556txd_lsc_valid){//HI556TXD LSC data
      if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &hi556txd_data_lsc[0], ptempbuf->u4Length)) {
        kfree(pBuff);
        kfree(pu1Params);
        pr_debug("ioctl copy to user failed\n");
        return -EFAULT;
      }
      i4RetValue = HI556TXD_LSC_DATA_SIZE;
      kfree(pBuff);
      kfree(pu1Params);
      return i4RetValue;
    }
    if (ptempbuf->sensorID == 0x557 && ptempbuf->u4Length == 0x10 && ptempbuf->u4Offset == 0x443 && hi556txd_awb_valid){//HI556TXD AWB data
      if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &hi556txd_data_awb[0], ptempbuf->u4Length)) {
        kfree(pBuff);
        kfree(pu1Params);
        pr_debug("ioctl copy to user failed\n");
        return -EFAULT;
      }
      i4RetValue = HI556TXD_AWB_DATA_SIZE;
      kfree(pBuff);
      kfree(pu1Params);
      return i4RetValue;
    }
    if (ptempbuf->sensorID == 0x557 && ptempbuf->u4Length == 0x05 && ptempbuf->u4Offset == 0x477 && hi556txd_af_valid){//HI556TXD AF data
      if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &hi556txd_data_af[0], ptempbuf->u4Length)) {
        kfree(pBuff);
        kfree(pu1Params);
        pr_debug("ioctl copy to user failed\n");
        return -EFAULT;
      ...
      return i4RetValue;
...
#endif
#if SC500CS_OTP
		if (ptempbuf->sensorID == 0xee27 && ptempbuf->u4Length == 4 && ptempbuf->u4Offset == 0x1){//SC500CS MID data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)&sc500cs_otp_data[1], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
		...
		if (ptempbuf->sensorID == 0xee27 && ptempbuf->u4Length == 0x74C && ptempbuf->u4Offset == SC500CS_LSC_51K_INFO_START){//SC500CS LSC 51K data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &sc500cs_otp_data[SC500CS_LSC_51K_INFO_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_err("ioctl copy to user failed\n");
				return -EFAULT;
			}
			i4RetValue = SC500CS_LSC_51K_DATA_SIZE;
			kfree(pBuff);
			kfree(pu1Params);
			return i4RetValue;
		}
		if (ptempbuf->sensorID == 0xee27 && ptempbuf->u4Length == SC500CS_AWB_51K_DATA_SIZE && ptempbuf->u4Offset == SC500CS_AWB_51K_INFO_START){//SC500CS AWB 51K data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &sc500cs_otp_data[SC500CS_AWB_51K_INFO_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_err("ioctl copy to user failed\n");
				return -EFAULT;
			}
			i4RetValue = SC500CS_AWB_51K_DATA_SIZE;
			kfree(pBuff);
			kfree(pu1Params);
			return i4RetValue;
		}
		if (ptempbuf->sensorID == 0xee27 && ptempbuf->u4Length == SC500CS_AF_MACRO_DATA_SIZE && ptempbuf->u4Offset == SC500CS_AF_MACRO_START){//SC500CS AF Macro data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &sc500cs_otp_data[SC500CS_AF_MACRO_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_err("ioctl copy to user failed\n");
				return -EFAULT;
			}
			i4RetValue = SC500CS_AF_MACRO_DATA_SIZE;
			kfree(pBuff);
			kfree(pu1Params);
			return i4RetValue;
		}
		if (ptempbuf->sensorID == 0xee27 && ptempbuf->u4Length == SC500CS_AF_INF_DATA_SIZE && ptempbuf->u4Offset == SC500CS_AF_INF_START){//SC500CS AF inf data
			if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *) &sc500cs_otp_data[SC500CS_AF_INF_START], ptempbuf->u4Length)) {
				kfree(pBuff);
				kfree(pu1Params);
				pr_err("ioctl copy to user failed\n");
				return -EFAULT;
			}
			i4RetValue = SC500CS_AF_INF_DATA_SIZE;
			kfree(pBuff);
			kfree(pu1Params);
			return i4RetValue;
		}
