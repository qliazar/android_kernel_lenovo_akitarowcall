...
static void sensor_init(void)
{
    ...
    write_cmos_sensor(0x3651,0x7d);
//    write_cmos_sensor(0x3652,0x33);
//    write_cmos_sensor(0x3654,0x01);
...
static struct stCAM_CAL_DATAINFO_STRUCT sc201cs_eeprom_data ={
	.sensorID= SC201CSMIPI_SENSOR_ID,
	.deviceID = 0x08,
	.dataLength = 0x0778,
...
static struct stCAM_CAL_CHECKSUM_STRUCT sc201cs_checksum[8] =
{
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		//+Bug 682590, zhoumin.wt, add, 2021.08.24, bring-up OTP for front and micro camera
/*
		size = imgSensorReadEepromData(&sc201cs_eeprom_data,sc201cs_checksum);
		if(size != sc201cs_eeprom_data.dataLength || (sc201cs_eeprom_data.sensorVendorid >> 24)!= read_eeprom(0x0001)) {
			pr_err("get eeprom data failed\n");
			if(sc201cs_eeprom_data.dataBuffer != NULL) {
				kfree(sc201cs_eeprom_data.dataBuffer);
				sc201cs_eeprom_data.dataBuffer = NULL;
			}
			*sensor_id = 0xFFFFFFFF;
			return ERROR_SENSOR_CONNECT_FAIL;
		} else {
			pr_debug("get eeprom data success\n");
			imgSensorSetEepromData(&sc201cs_eeprom_data);
		}
*/
		//+Bug 682590, zhoumin.wt, add, 2021.08.24, bring-up OTP for front and micro camera		
...
UINT32 SC201CS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
