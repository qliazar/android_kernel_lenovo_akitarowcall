...
#include "sc500csmipiraw_Sensor.h"
...
//+bug652822,youjintong.wt,ADD,2021/05/12,add TMO(4G) OTP&PDAF bring up code.
#define SC500CS_OTP  1

#if SC500CS_OTP
#define SC500CS_OTP_SIZE 0x077E + 1
unsigned char sc500cs_otp_data[SC500CS_OTP_SIZE];
#define MODULE_INFO_FLAG 0
#define MODULE_INFO_CHECKSUM 8
unsigned char sc500cs_module_info_data[7] = {0};

#define AWB_51K_INFO_FLAG 0x017
#define AWB_51K_INFO_CHECKSUM 0x28
unsigned char sc500cs_awb_51k_data_valid = 0;

#define AF_51K_INFO_FLAG 0x029
#define AF_51K_INFO_CHECKSUM 0x2F
unsigned char sc500cs_af_51k_data_valid = 0;

#define LSC_51K_INFO_FLAG 0x30
#define LSC_51K_INFO_CHECKSUM 0x77D
unsigned char sc500cs_lsc_51k_data_valid = 0;
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
 ...
 { 2592, 1944,	  16,	12, 2560, 1920, 640, 480, 0000, 640, 480,    0,	0, 640, 480}, //hight speed video 
...
kal_uint16 addr_data_pair_init_sc500cs[] = {
	...
	0x4008,0x00,
	0x4009,0xc8,
	0x440e,0x02, //Bug 682590, zhoumin.wt, add, 2021/08/27,  update init register
	0x4509,0x28,
	0x4837,0x21,
	0x5000,0x0e,
	0x0100,0x01,
...
static void sensor_init(void)
{
	...
	write_cmos_sensor8(0x3221,0x66);//mirror and flip
	...
	//+Bug 682590, zhoumin.wt, add, 2021/08/26,  modify for mipi test fail
	write_cmos_sensor8(0x3652,0x33);
	write_cmos_sensor8(0x3654,0x01);
	write_cmos_sensor8(0x481b,0x39);
	...
	write_cmos_sensor8(0x440e,0x02); //Bug 682590, zhoumin.wt, add, 2021/08/27,  update init register
...
kal_uint16 addr_data_pair_capture_30fps_sc500cs[] = {
	0x0100,0x00,
	0x0100,0x01,
	0x302d,0x00
};
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_normal_video_sc500cs[] = {
	0x0100,0x00,
	0x0100,0x01,
	0x302d,0x00
};
...
write_cmos_sensor8(0x3106,0x01);
write_cmos_sensor8(0x3200,0x02);
write_cmos_sensor8(0x3201,0x94);
write_cmos_sensor8(0x3202,0x01);
write_cmos_sensor8(0x3203,0xf0);
write_cmos_sensor8(0x3204,0x07);
write_cmos_sensor8(0x3205,0x9b);
...
//+bug652822,youjintong.wt,ADD,2021/05/12,add TMO(4G) OTP&PDAF bring up code.
#if SC500CS_OTP
int read_sc500cs_otp(void){
		int addr = imgsensor.i2c_write_id;
		int i, sum = 0;
		imgsensor.i2c_write_id = 0xA0;
		for(i = 0; i < SC500CS_OTP_SIZE; i++){
			sc500cs_otp_data[i] = read_cmos_sensor(i);
			pr_err("=== sc500cs_otp_data[%x]: %x ===\n", i, sc500cs_otp_data[i]);
		}
		imgsensor.i2c_write_id = addr;
		//check module info
		for(i = MODULE_INFO_FLAG; i < MODULE_INFO_CHECKSUM; i++){
			sum += sc500cs_otp_data[i];
		}
		if((sc500cs_otp_data[MODULE_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == sc500cs_otp_data[MODULE_INFO_CHECKSUM]){
			LOG_INF("asdf Mouule   ID 0x%x ", sc500cs_otp_data[1]);
			LOG_INF("Position ID 0x%x ", sc500cs_otp_data[2]);
			LOG_INF("Lens     ID 0x%x ", sc500cs_otp_data[3]);
			LOG_INF("VCM      ID 0x%x ", sc500cs_otp_data[4]);
			LOG_INF("Year: %d Month: %d Data: %d", sc500cs_otp_data[5], sc500cs_otp_data[6], sc500cs_otp_data[7]);
		}else{
			pr_err("=== sc500cs_read module info failed ===\n");
			return 0;
		}
		sum = 0;
		...
		if((sc500cs_otp_data[AWB_51K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == sc500cs_otp_data[AWB_51K_INFO_CHECKSUM]){
			sc500cs_awb_51k_data_valid = 1;
		}else{
			pr_err("=== asdf sc500cs_read module info awb 51k failed ===\n");
			return 0;
		}
		sum = 0;
		//check module af  51k info
		for(i = AF_51K_INFO_FLAG; i < AF_51K_INFO_CHECKSUM; i++){
			sum += sc500cs_otp_data[i];
		}
		if((sc500cs_otp_data[AF_51K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == sc500cs_otp_data[AF_51K_INFO_CHECKSUM]){
			sc500cs_af_51k_data_valid = 1;
		}else{
			pr_err("=== asdf sc500cs_read module info af 51k failed ===\n");
			return 0;
		}
		sum = 0;
		...
		if((sc500cs_otp_data[LSC_51K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == sc500cs_otp_data[LSC_51K_INFO_CHECKSUM]){
			sc500cs_lsc_51k_data_valid = 1;
		}else{
			pr_err("=== sc500cs_read module info lsc 51k failed ===\n");
			return 0;
		}
		sum = 0;
...
#endif
//-bug652822,youjintong.wt,ADD,2021/05/12,add TMO(4G) OTP&PDAF bring up code.
				pr_err("sc500cs i2c write id : 0x%x, sensor id: 0x%x\n",
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		pr_err("sc500cs Read id fail,sensor id: 0x%x\n", *sensor_id);
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	return ERROR_NONE;
