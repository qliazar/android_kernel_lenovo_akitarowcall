...
#define PFX "hi556txd_camera_sensor"
...
static kal_uint16 hi556txd_table_write_cmos_sensor(
...
unsigned char hi556txd_data_af[AF_DATA_SIZE + 1] = {0};/*Add check sum*/
unsigned char hi556txd_data_lsc[LSC_DATA_SIZE + 1] = {0};/*Add check sum*/
unsigned char hi556txd_data_awb[AWB_DATA_SIZE + 1] = {0};/*Add check sum*/
unsigned char hi556txd_data_info[MODULE_INFO_SIZE + 1] = {0};/*Add check sum*/
unsigned char hi556txd_module_id = 0;
unsigned char hi556txd_lsc_valid = 0;
unsigned char hi556txd_awb_valid = 0;
unsigned char hi556txd_af_valid = 0;

static void hi556txd_disable_otp_func(void)
{
...
static int read_hi556txd_module_info(void){
  ...
  if(otp_grp_flag == 0x01)
    ...
    LOG_INF("no OTP hi556txd_data_info\n");
    return 0;
  }
  if(minfo_start_addr != 0){
    ...
    for(i = 0; i < MODULE_INFO_SIZE + 1; i++){
      hi556txd_data_info[i]=read_cmos_sensor(0x0108);
    }
    for(i = 0; i < MODULE_INFO_SIZE; i++){
      check_sum_cal += hi556txd_data_info[i];
    }

    check_sum_cal = (check_sum_cal % 255) + 1;
    hi556txd_module_id = hi556txd_data_info[0];
    position = hi556txd_data_info[1];
    lens_id = hi556txd_data_info[2];
    vcm_id = hi556txd_data_info[3];
    year = hi556txd_data_info[4];
    month = hi556txd_data_info[5];
    day = hi556txd_data_info[6];
    check_sum = hi556txd_data_info[MODULE_INFO_SIZE];
  }
  //LOG_INF("module_id=0x%x position=0x%x\n", hi556txd_module_id, position);
  LOG_INF("=== HI556TXD INFO module_id=0x%x position=0x%x ===\n", hi556txd_module_id, position);
  LOG_INF("=== HI556TXD INFO lens_id=0x%x,vcm_id=0x%x ===\n",lens_id, vcm_id);
  LOG_INF("=== HI556TXD INFO date is %d-%d-%d ===\n",year,month,day);
  LOG_INF("=== HI556TXD INFO check_sum=0x%x,check_sum_cal=0x%x ===\n", check_sum, check_sum_cal);
...
static int read_hi556txd_awb_info(void){
  ...
  int i;
  /* awb group 1 */
  write_cmos_sensor_8(0x010a,((0x0442)>>8)&0xff);
  write_cmos_sensor_8(0x010b,(0x0442)&0xff);
  write_cmos_sensor_8(0x0102,0x01);
  otp_grp_flag = read_cmos_sensor(0x0108);
  LOG_INF("otp_grp_flag = 0x%x\n",otp_grp_flag);
  if(otp_grp_flag == 0x01)
    awb_start_addr = 0x0443;
  else if (otp_grp_flag == 0x13)
    awb_start_addr = 0x0454;
  else if(otp_grp_flag == 0x37)
    awb_start_addr = 0x0465;
  else{
  ...
  if(awb_start_addr != 0)
  {
    ...
    for(i = 0; i < AWB_DATA_SIZE + 1; i++){
      hi556txd_data_awb[i]=read_cmos_sensor(0x0108);
    }
    for(i = 0; i < AWB_DATA_SIZE; i++){
      check_sum_awb_cal += hi556txd_data_awb[i];
    }
    LOG_INF("check_sum_awb_cal =0x%x \n",check_sum_awb_cal);
    r = ((hi556txd_data_awb[1]<<8)&0xff00)|(hi556txd_data_awb[0]&0xff);
    b = ((hi556txd_data_awb[3]<<8)&0xff00)|(hi556txd_data_awb[2]&0xff);
    gr = ((hi556txd_data_awb[5]<<8)&0xff00)|(hi556txd_data_awb[4]&0xff);
    gb = ((hi556txd_data_awb[7]<<8)&0xff00)|(hi556txd_data_awb[6]&0xff);
    golden_r = ((hi556txd_data_awb[9]<<8)&0xff00)|(hi556txd_data_awb[8]&0xff);
    golden_b = ((hi556txd_data_awb[11]<<8)&0xff00)|(hi556txd_data_awb[10]&0xff);
    golden_gr = ((hi556txd_data_awb[13]<<8)&0xff00)|(hi556txd_data_awb[12]&0xff);
    golden_gb = ((hi556txd_data_awb[15]<<8)&0xff00)|(hi556txd_data_awb[14]&0xff);
    check_sum_awb = hi556txd_data_awb[AWB_DATA_SIZE];
    check_sum_awb_cal = (check_sum_awb_cal % 255) + 1;
  }
  LOG_INF("=== HI556TXD AWB r=0x%x, b=0x%x, gr=%x, gb=0x%x ===\n", r, b,gb, gr);
  LOG_INF("=== HI556TXD AWB gr=0x%x,gb=0x%x,gGr=%x, gGb=0x%x ===\n", golden_r, golden_b, golden_gr, golden_gb);
  LOG_INF("=== HI556TXD AWB check_sum_awb=0x%x,check_sum_awb_cal=0x%x ===\n",check_sum_awb,check_sum_awb_cal);
...
static int read_hi556txd_lsc_info(void){
  int otp_grp_flag = 0, lsc_start_addr = 0;
  int check_sum_lsc = 0, check_sum_lsc_cal = 0;
  int i;
  /* lsc group 1 */
  write_cmos_sensor_8(0x010a,((0x0489)>>8)&0xff);
  write_cmos_sensor_8(0x010b,(0x0489)&0xff);
  write_cmos_sensor_8(0x0102,0x01);
  otp_grp_flag = read_cmos_sensor(0x0108);
  LOG_INF("otp_grp_flag = 0x%x\n",otp_grp_flag);
  if(otp_grp_flag == 0x01)
    lsc_start_addr = 0x048a;
  else if (otp_grp_flag == 0x13)
    lsc_start_addr = 0x0bd7;
  else if(otp_grp_flag == 0x37)
    lsc_start_addr = 0x1324;
  else{
  ...
  if(lsc_start_addr != 0){
    ...
    {
      hi556txd_data_lsc[i] = read_cmos_sensor(0x0108);
    }
    for(i = 0; i < LSC_DATA_SIZE; i++){
      check_sum_lsc_cal += hi556txd_data_lsc[i];
    }
    LOG_INF("check_sum_awb_cal =0x%x \n",check_sum_lsc_cal);
    check_sum_lsc = hi556txd_data_lsc[LSC_DATA_SIZE];
    check_sum_lsc_cal = (check_sum_lsc_cal % 255) + 1;
  }
  LOG_INF("=== HI556TXD LSC check_sum_awb=0x%x, check_sum_awb_cal=0x%x ===\n", check_sum_lsc, check_sum_lsc_cal);
...
static int read_hi556txd_af_info(void){
  int otp_grp_flag = 0, af_start_addr = 0;
  int check_sum_af = 0, check_sum_af_cal = 0;
  int i;
  /* lsc group 1 */
  write_cmos_sensor_8(0x010a,((0x476)>>8)&0xff);
  write_cmos_sensor_8(0x010b,(0x476)&0xff);
  write_cmos_sensor_8(0x0102,0x01);
  otp_grp_flag = read_cmos_sensor(0x0108);
  LOG_INF("lsc_otp_grp1_flag = 0x%x\n",otp_grp_flag);
  if(otp_grp_flag == 0x01)
    af_start_addr = 0x0477;
  else if (otp_grp_flag == 0x13)
    af_start_addr = 0x047d;
  else if(otp_grp_flag == 0x37)
      af_start_addr = 0x0483;
    else{
  ...
  if(af_start_addr != 0){
    ...
    {
      hi556txd_data_af[i] = read_cmos_sensor(0x0108);
      LOG_INF("hi556txd_data_af[%d]=0x%x\n",i,hi556txd_data_af[i]);
    }
    for(i = 0; i < AF_DATA_SIZE; i++){
      check_sum_af_cal += hi556txd_data_af[i];
    }
    LOG_INF("check_sum_af_cal =0x%x \n",check_sum_af_cal);
    check_sum_af = hi556txd_data_af[AF_DATA_SIZE];
...
static int hi556txd_sensor_otp_info(void){
	int ret = 0;
	...
	/* 3. read eeprom data */
	//minfo && awb group
  ret = read_hi556txd_module_info();
  if(ret != 1){
    hi556txd_module_id = 0;
    LOG_ERR("=== hi556txd_data_info invalid ===\n");
  }
  ret = read_hi556txd_awb_info();
  if(ret != 1){
    hi556txd_awb_valid = 0;
    LOG_ERR("=== hi556txd_data_awb invalid ===\n");
  }else{
    hi556txd_awb_valid = 1;
  }
  ret = read_hi556txd_lsc_info();
  if(ret != 1){
    hi556txd_lsc_valid = 0;
    LOG_ERR("=== hi556txd_data_lsc invalid ===\n");
  }else{
    hi556txd_lsc_valid = 1;
  }

  ret = read_hi556txd_af_info();
  if(ret != 1){
    hi556txd_af_valid = 0;
    LOG_ERR("=== hi556txd_data_af invalid ===\n");
  }else{
    hi556txd_af_valid = 1;
  }

	/* 4. disable otp function */
	hi556txd_disable_otp_func();
  if(hi556txd_module_id == 0 || hi556txd_lsc_valid == 0 || hi556txd_awb_valid == 0 ||hi556txd_af_valid == 0){
    return 0;
  }else{
    return 1;
  }
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_init_hi556txd[] = {
	0x0a00, 0x0000,
	0x0e00, 0x0102,
	0x0e02, 0x0102,
	0x0e0c, 0x0100,
...
{
#if MULTI_WRITE
	hi556txd_table_write_cmos_sensor(
		addr_data_pair_init_hi556txd,
		sizeof(addr_data_pair_init_hi556txd) /
		sizeof(kal_uint16));
#else
	...
	write_cmos_sensor(0x0958, 0xdb80);
	write_cmos_sensor(0x095a, 0x5dfe);
	...
	write_cmos_sensor(0x004e, 0x0100); //PFC_enable
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_preview_hi556txd[] = {
	0x0a00, 0x0000,
	0x0b0a, 0x8259,
	0x0f30, 0x6e25,
	0x0f32, 0x7167,
...
#if MULTI_WRITE
	hi556txd_table_write_cmos_sensor(
		addr_data_pair_preview_hi556txd,
		sizeof(addr_data_pair_preview_hi556txd) /
		sizeof(kal_uint16));
#else
	//Sensor Information////////////////////////////
	//Sensor	  : hi-556
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_capture_fps_hi556txd[] = {
	0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7067,
	0x004a, 0x0100,
...
kal_uint16 addr_data_pair_capture_30fps_hi556txd[] = {
	0x0a00, 0x0000,
	0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7067,
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 300) {
	hi556txd_table_write_cmos_sensor(
		addr_data_pair_capture_30fps_hi556txd,
		sizeof(addr_data_pair_capture_30fps_hi556txd) /
		sizeof(kal_uint16));
	} else {
	hi556txd_table_write_cmos_sensor(
		addr_data_pair_capture_fps_hi556txd,
		sizeof(addr_data_pair_capture_fps_hi556txd) /
		sizeof(kal_uint16));
	}
#else
	if (currefps == 300) {
		...
		write_cmos_sensor(0x003e, 0x0000);
		write_cmos_sensor(0x0074, 0x0821);
		write_cmos_sensor(0x0070, 0x0410);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_hs_video_hi556txd[] = {
	0x0a00, 0x0000,
	0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7267,
...
#if MULTI_WRITE
	hi556txd_table_write_cmos_sensor(
		addr_data_pair_hs_video_hi556txd,
		sizeof(addr_data_pair_hs_video_hi556txd) /
		sizeof(kal_uint16));
#else
	write_cmos_sensor(0x0a00, 0x0000);
	write_cmos_sensor(0x0b0a, 0x8252);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_slim_video_hi556txd[] = {
	0x0a00, 0x0000,
	0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7167,
...
#if MULTI_WRITE
	hi556txd_table_write_cmos_sensor(
		addr_data_pair_slim_video_hi556txd,
		sizeof(addr_data_pair_slim_video_hi556txd) /
		sizeof(kal_uint16));
#else
	write_cmos_sensor(0x0a00, 0x0000);
	write_cmos_sensor(0x0b0a, 0x8252);
...
UINT32 HI556TXD_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc =  &sensor_func;
	return ERROR_NONE;
}	/*	HI556TXD_MIPI_RAW_SensorInit	*/
