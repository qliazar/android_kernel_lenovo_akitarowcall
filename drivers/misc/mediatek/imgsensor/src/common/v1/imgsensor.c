...
#define __I2C_STATE_CONTROL_FEATURE__
...
#ifdef __I2C_STATE_CONTROL_FEATURE__
struct pinctrl       *i2c4_pinctrl = NULL;
struct mutex       *i2c4_pinctrl_mutex = NULL;
struct pinctrl_state *i2c4_scl_pin_en0 = NULL;
struct pinctrl_state *i2c4_scl_pin_en1 = NULL;
struct pinctrl_state *i2c4_sda_pin_en0 = NULL;
struct pinctrl_state *i2c4_sda_pin_en1 = NULL;
#endif
...
imgsensor_sensor_open(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if((NULL != i2c4_pinctrl) && !strcmp((char *)psensor_inst->psensor_name, "sc201cs_mipi_raw")  ){
		pr_info("[%s] : pinctrl_select_state enable  i2c4 pin for [%d][%s]\n",
					__func__,
					psensor_inst->sensor_idx,
					psensor_inst->psensor_name);
		mutex_lock(i2c4_pinctrl_mutex);
		ret = pinctrl_select_state(i2c4_pinctrl, i2c4_scl_pin_en1);
		if(ret < 0){
			pr_err("%s pinctrl_select_state enable  i2c4 scl pin  failed ! \n", __func__);
			//return ret;
		} else {
			pr_info("%s pinctrl_select_state enable  i2c4 scl pin succeed ! \n", __func__);
		}
		mutex_unlock(i2c4_pinctrl_mutex);

		mutex_lock(i2c4_pinctrl_mutex);
		ret = pinctrl_select_state(i2c4_pinctrl, i2c4_sda_pin_en1);
		if(ret < 0){
			pr_err("%s pinctrl_select_state enable  i2c4 sda pin failed !\n", __func__);
			//return ret;
		} else {
			pr_info("%s pinctrl_select_state enable  i2c4 sda pin succeed !\n", __func__);
		}
		mutex_unlock(i2c4_pinctrl_mutex);
	}
...
imgsensor_sensor_close(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if((NULL != i2c4_pinctrl) && !strcmp((char *)psensor_inst->psensor_name, "sc201cs_mipi_raw")){
		...
		pr_info("[%s] : pinctrl_select_state disable  i2c4 pin for [%d][%s]\n",
					__func__,
					psensor_inst->sensor_idx,
					psensor_inst->psensor_name);
		mutex_lock(i2c4_pinctrl_mutex);
		ret = pinctrl_select_state(i2c4_pinctrl, i2c4_scl_pin_en0);
		if(ret < 0){
			pr_err("%s pinctrl_select_state disable  i2c4 scl pin  failed ! \n", __func__);
			//return ret;
		} else {
			pr_info("%s pinctrl_select_state disable  i2c4 scl pin succeed ! \n", __func__);
		}
		mutex_unlock(i2c4_pinctrl_mutex);
		mutex_lock(i2c4_pinctrl_mutex);
		ret = pinctrl_select_state(i2c4_pinctrl, i2c4_sda_pin_en0);
		if(ret < 0){
			pr_err("%s pinctrl_select_state disable  i2c4 sda pin failed !\n", __func__);
			//return ret;
		} else {
			pr_info("%s pinctrl_select_state disable  i2c4 sda pin succeed !\n", __func__);
		}
		mutex_unlock(i2c4_pinctrl_mutex);
	}
	imgsensor_mutex_unlock(psensor_inst);
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	#ifdef __I2C_STATE_CONTROL_FEATURE__
	if((NULL != i2c4_pinctrl) && ((psensor_inst->sensor_idx == 1) && !strcmp((char *)psensor_inst->psensor_name, "sc201cs_mipi_raw"))){
		pr_info("[%s] : pinctrl_select_state enable  i2c4 pin for [%d][%s]\n",
					__func__,
					psensor_inst->sensor_idx,
					psensor_inst->psensor_name);
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		...
		if(!strcmp(psensor_inst->psensor_name, "s5k4h7_mipi_raw")){
			...
			hardwareinfo_set_prop(HARDWARE_FRONT_CAM_MOUDULE_ID, "CXT");
                }else if(!strcmp(psensor_inst->psensor_name, "sc201cs_mipi_raw")){
			hardwareinfo_set_prop(HARDWARE_FRONT_CAM, "SC201CS");
...
#ifdef __I2C_STATE_CONTROL_FEATURE__
	if (NULL != gpimgsensor_hw_platform_device) {
		i2c4_pinctrl_mutex = &pinctrl_mutex;
		i2c4_pinctrl = devm_pinctrl_get(&(gpimgsensor_hw_platform_device->dev));
		if(IS_ERR(i2c4_pinctrl)){
			pr_err("i2c4_pinctrl get pinctrl failed !");
			//return -1;
		} else {
			pr_info("i2c4_pinctrl get pinctrl succeed");
		}
		i2c4_scl_pin_en0 = pinctrl_lookup_state(i2c4_pinctrl, "i2c4_pin_scl_pull_en0");
		if (IS_ERR(i2c4_scl_pin_en0)) {
			pr_err("i2c4_pinctrl pinctrl_lookup_state scl_pull_en0 failed !");
			//return -1;
		} else {
			pr_info("i2c4_pinctrl pinctrl_lookup_state scl_pull_en0 succeed");
		}
		i2c4_scl_pin_en1 = pinctrl_lookup_state(i2c4_pinctrl, "i2c4_pin_scl_pull_en1");
		if (IS_ERR(i2c4_scl_pin_en1)) {
			pr_err("i2c4_pinctrl pinctrl_lookup_state scl_pull_en1 failed !");
			//return -1;
		} else {
			pr_info("i2c4_pinctrl pinctrl_lookup_state scl_pull_en1 succeed");
		}
		i2c4_sda_pin_en0 = pinctrl_lookup_state(i2c4_pinctrl, "i2c4_pin_sda_pull_en0");
		if (IS_ERR(i2c4_sda_pin_en0)) {
			pr_err("i2c4_pinctrl pinctrl_lookup_state sda_pull_en0 failed !");
			//return -1;
		} else {
			pr_info("i2c4_pinctrl pinctrl_lookup_state sda_pull_en0 succeed");
		}
		i2c4_sda_pin_en1 = pinctrl_lookup_state(i2c4_pinctrl, "i2c4_pin_sda_pull_en1");
		if (IS_ERR(i2c4_sda_pin_en1)) {
			pr_err("i2c4_pinctrl pinctrl_lookup_state sda_pull_en1 failed !");
			//return -1;
		} else {
			pr_info("i2c4_pinctrl pinctrl_lookup_state sda_pull_en1 succeed");
		}
	}
#endif
