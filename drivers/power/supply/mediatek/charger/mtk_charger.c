...
int maintain_mode_cv_handle(int BATTERY_MAINTAIN_MODE)
{
	int cv =0;
	if(BATTERY_MAINTAIN_MODE == 0)
		cv = 4432000;//100 percent : 4432000
	if(BATTERY_MAINTAIN_MODE == 1)
		cv = 4408500;//95 percent : 4318500
	if(BATTERY_MAINTAIN_MODE == 2)
		cv = 4368500;//90 percent : 4233500
	if(BATTERY_MAINTAIN_MODE == 3)
		cv = 4283500;//80 percent : 4143380
	chr_err("fenglin battery soc = %d\n",battery_get_soc());
	return cv;
}
...
void do_sw_jeita_state_machine(struct charger_manager *info)
{
	...
	if (info->battery_temp >= info->data.temp_t4_thres) {
		...
		month_used = get_used_month(factory_time,now_time);
		BATTERY_MAINTAIN_MODE = get_battery_maintain_mode(month_used);
		chr_err("maintain_mode = %d\n",BATTERY_MAINTAIN_MODE);  
	...
	if (sw_jeita->sm == TEMP_ABOVE_T4){
		...
		sw_jeita->cv = maintain_mode_cv_handle(BATTERY_MAINTAIN_MODE);
...
//	struct switch_charging_alg_data *swchgalg = info->algorithm_data;
	int limit_soc, ret;
		...
		chr_err("ato_charger_now_uisoc:%d\n",limit_soc);

	if (battery_get_debug_uisoc() != 0xffff)
		limit_soc = battery_get_debug_uisoc();

	if(limit_soc >= 80) {
		ret = charger_dev_hz_mode(info->chg1_dev, 1);
		if (ret == -ENOTSUPP )
			ret = charger_dev_enable_powerpath(info->chg1_dev,false);
		_mtk_charger_do_charging(info, 0);
		chr_err("ato_charger_limit_soc:%d disable charging \n",limit_soc);
	}

	if(limit_soc <= 60) {
		ret = charger_dev_hz_mode(info->chg1_dev, 0);
		if (ret == -ENOTSUPP )
                        ret = charger_dev_enable_powerpath(info->chg1_dev,true);
		_mtk_charger_do_charging(info, 1);
		chr_err("ato_charger_limit_soc:%d enable charging \n",limit_soc);
	}
}
#endif
