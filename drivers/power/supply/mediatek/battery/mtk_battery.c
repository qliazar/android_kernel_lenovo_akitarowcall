...
/* add for meta tool----------------------------------------- */
static int month_used;
static struct mytime factory_time;
static int BATTERY_MAINTAIN_MODE;
static struct mytime now_time;

static int maintainmode_status;
...
extern int get_use_month(struct mytime factory_time,struct mytime now_time);
extern int get_bat_maintain_mode(int month_used);
...
static int maintain_mode_soc_handle(struct battery_data *bat_data)
{
	maintainmode_status = bat_data->battery_maintain;
	month_used = get_use_month(factory_time,now_time);
	BATTERY_MAINTAIN_MODE = get_bat_maintain_mode(month_used);
	if(maintainmode_status != 0 && BATTERY_MAINTAIN_MODE == 1)
		{
			if(battery_get_soc() * 1.08 >= 100)
			bat_data->BAT_CAPACITY = 100;
			else
			bat_data->BAT_CAPACITY = battery_get_soc() * 1.08;
		}
	if(maintainmode_status != 0 && BATTERY_MAINTAIN_MODE == 2)
		{
			if(battery_get_soc() * 1.13 >= 100)
			bat_data->BAT_CAPACITY = 100;
			else
			bat_data->BAT_CAPACITY = battery_get_soc() * 1.13;
		}
	if(maintainmode_status != 0 && BATTERY_MAINTAIN_MODE == 3)
		{
			if(battery_get_soc() * 1.28 >= 100)
			bat_data->BAT_CAPACITY = 100;
			else
			bat_data->BAT_CAPACITY = battery_get_soc() * 1.28;
		}
	return bat_data->BAT_CAPACITY;
}
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (gm.fixed_uisoc != 0xffff)
			...
			val->intval = maintain_mode_soc_handle(data);
		break;
	...
	case POWER_SUPPLY_PROP_BATTERY_MAINTAIN:
		val->intval = data->battery_maintain;
                break;
	case POWER_SUPPLY_PROP_BATTERY_PROTECTED:
		val->intval = data->battery_protected;
                break;
	default:
		ret = -EINVAL;
		break;
...
static ssize_t battery_maintain_show_property(struct device *dev,
					  struct device_attribute *attr,
					  char *buf)
{
	return sprintf(buf, "%d\n", battery_main.battery_maintain);

}
static ssize_t battery_maintain_store_property(struct device *dev,
					   struct device_attribute *attr,
					   const char *buf, size_t count) {
	int val;
	kstrtoint(buf, 10, &val);
	
	battery_main.battery_maintain = val;
	return count;
}
static DEVICE_ATTR(battery_maintain, 0664, battery_maintain_show_property, battery_maintain_store_property);

static ssize_t battery_protected_show_property(struct device *dev,
					  struct device_attribute *attr,
					  char *buf)
{
	return sprintf(buf, "%d\n", battery_main.battery_protected);

}
static ssize_t battery_protected_store_property(struct device *dev,
					   struct device_attribute *attr,
					   const char *buf, size_t count) {
	int val;
	kstrtoint(buf, 10, &val);
	
	battery_main.battery_protected = val;
	return count;
}
static DEVICE_ATTR(battery_protected, 0664, battery_protected_show_property, battery_protected_store_property);
...
/*****************************/
	...
	ret = device_create_file(&battery_main.psy->dev, &dev_attr_battery_maintain);
	ret = device_create_file(&battery_main.psy->dev, &dev_attr_battery_protected);
