...
#define HX9031A_DRIVER_VER "Change-Id 203000"
...
#endif
                input_report_key(input,hx9031a_channels[ii].code,1);
                input_sync(input);
...
/*wt factory caiqiao*/
                if(1 == ii){
                       hx9031a_wifi_status = 254;
                      }else if(0 == ii){
                       hx9031a_tel_status = 253;
                      }
/*wt factory caiqiao*/
            ...
            if (channel_p->state == IDLE)
                ...
                wake_lock_timeout(&hx9031a_wake_lock, HZ * 1);
#endif
                input_report_key(input,hx9031a_channels[ii].code, 0);
                input_sync(input);
                channel_p->state = IDLE;
/*wt factory caiqiao*/
                 if(1 == ii){
                       hx9031a_wifi_status =198; 
                      }else if(0 == ii){
                       hx9031a_tel_status = 199;
                      }
/*wt factory caiqiao*/
    ...
    for (ii = 0; ii < ARRAY_SIZE(hx9031a_channels); ii++) {
        if (strcmp(sensors_cdev->name, hx9031a_channels[ii].name) == 0) {
            if (1 == enable) {
                ...
                wake_lock_timeout(&hx9031a_wake_lock, HZ * 1);
#endif
                input_report_key(hx9031a_channels[ii].hx9031a_input_dev,hx9031a_channels[ii].code, 0);
                ...
                wake_lock_timeout(&hx9031a_wake_lock, HZ * 1);
#endif
                input_report_key(hx9031a_channels[ii].hx9031a_input_dev,hx9031a_channels[ii].code, -1);
                input_sync(hx9031a_channels[ii].hx9031a_input_dev);
            } else {
		...
		const char *buf, size_t count)
{
	...
	int ii = 0;
        ...
        printk("write_enable_store end\n");
   	return count;
}

static CLASS_ATTR(enable, 0664, NULL, write_enable_store);
...
//}end =============================================================
    ...
    for (ii = 0; ii < ARRAY_SIZE(hx9031a_channels); ii++) {
        if ((hx9031a_pdata.channel_used_flag >> ii) & 0x1) { //used flag的值来自dts，表示通道有效性，每一位表示一个通道。
            ...
            __set_bit(hx9031a_channels[ii].code, hx9031a_channels[ii].hx9031a_input_dev->keybit);
...
/*WT factory use  caiqiao*/
        ret = class_register(&sar_class);
	if (ret < 0) {
		printk("Create sysfs class failed (%d)\n", ret);
		goto failed_class_register; 
	}
        ret = class_create_file(&sar_class, &class_attr_wifi_status);
	if (ret < 0) {
		printk("Create wifi_status file failed (%d)\n", ret);
		goto failed_class_register;
	}
	ret = class_create_file(&sar_class, &class_attr_tel_status);
	if (ret < 0) {
		printk("Create tel_status file failed (%d)\n", ret);
		goto failed_class_register;
	}
	ret = class_create_file(&sar_class, &class_attr_enable);
	if (ret < 0) {
...
#ifdef WT_ADD_SAR_HARDWARE_INFO
        snprintf(firmware_ver,HARDWARE_MAX_ITEM_LONGTH,"HX9023E");
