 ...
 *
 * $Revision: 84770 $
 * $Date: 2021-07-06 21:15:53 +0800 (週二, 06 七月 2021) $
 *
...
static struct proc_dir_entry *NVT_proc_tp_openshort_entry = NULL;
...
//	NVT_LOG("++\n");
	...
	if ((!TestResult_Short) && (!TestResult_Open) &&
			(!TestResult_FW_Rawdata) && (!TestResult_FW_CC) && (!TestResult_Noise) && (!TestResult_Pen_FW_Raw) && (!TestResult_Pen_Noise)) {
...
static const struct file_operations nvt_tp_openshort_fops = {
	.owner = THIS_MODULE,
	.open = nvt_selftest_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
int32_t nvt_mp_proc_init(void)
{
	...
	proc_android_touch_entry = proc_mkdir("touchscreen", NULL);
	if (proc_android_touch_entry == NULL) {
		NVT_ERR("create /proc/touchscreen Failed!\n");
		return -1;
	}
	...
	//----ctp_openshort_test----
	NVT_proc_tp_openshort_entry = proc_create("ctp_openshort_test", 0444, proc_android_touch_entry, &nvt_tp_openshort_fops);
	if (NVT_proc_tp_openshort_entry == NULL) {
		NVT_ERR("create /proc/touchscreen/ctp_openshort_test Failed!\n");
		return -1;
	}
	...
	if(nvt_mp_buffer_init()) {
		...
		NVT_LOG("create mp node Succeeded!\n");
	}
	return 0;
...
void nvt_mp_proc_deinit(void)
{
	...
	if (NVT_proc_tp_openshort_entry != NULL) {
		remove_proc_entry("touchscreen/ctp_openshort_test", NULL);
		NVT_proc_tp_openshort_entry = NULL;
		NVT_LOG("Removed /proc/%s\n", "touchscreen/ctp_openshort_test");
	}
}
