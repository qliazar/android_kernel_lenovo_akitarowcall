...
#if WAKEUP_GESTURE
#define GESTURE_MODE "nvt_gesture_mode"
#endif
...
static ssize_t nvt_gesture_store(struct file *file, const char *buffer, size_t count, loff_t *pos) {
	...
	if(state == 0)
		ntp_gesture_mode = false;
	else
		ntp_gesture_mode = true;
...
#if WAKEUP_GESTURE
	NVT_proc_gesture_entry = proc_create(GESTURE_MODE, 0644, NULL, &nvt_gesture_fops);
...
#if WAKEUP_GESTURE
	if (NVT_proc_gesture_entry != NULL) {
		remove_proc_entry(GESTURE_MODE, NULL);
