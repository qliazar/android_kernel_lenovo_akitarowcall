...
*****************************************************************************/
#define FTS_DRIVER_NAME                     "ftsm_ts"
...
*****************************************************************************/
int ftsm_wait_tp_to_valid(void)
{
    int ret = 0;
    int cnt = 0;
    u8 idh = 0;
    ...
    do {
        ret = ftsm_read_reg(FTS_REG_CHIP_ID, &idh);
...
/*****************************************************************************
*  Name: ftsm_tp_state_recovery
*  Brief: Need execute this function when reset
*  Input:
*  Output:
*  Return:
*****************************************************************************/
void ftsm_tp_state_recovery(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    /* wait tp stable */
    ftsm_wait_tp_to_valid();
    /* recover TP charger state 0x8B */
    /* recover TP glove state 0xC0 */
    /* recover TP cover state 0xC1 */
    ftsm_ex_mode_recovery(ts_data);
#if FTS_PSENSOR_EN
    fts_proximity_recovery(ts_data);
#endif

    /* recover TP gesture state 0xD0 */
    ftsm_gesture_recovery(ts_data);
    FTS_FUNC_EXIT();
}

int ftsm_reset_proc(int hdelayms)
{
    FTS_DEBUG("tp reset");
    tpd_gpio_output(ftsm_data->pdata->reset_gpio, 0);
    msleep(1);
    tpd_gpio_output(ftsm_data->pdata->reset_gpio, 1);
    if (hdelayms) {
        msleep(hdelayms);
    }
...
void ftsm_irq_disable(void)
{
    unsigned long irqflags;

    FTS_FUNC_ENTER();
    spin_lock_irqsave(&ftsm_data->irq_lock, irqflags);

    if (!ftsm_data->irq_disabled) {
        disable_irq_nosync(ftsm_data->irq);
        ftsm_data->irq_disabled = true;
    }

    spin_unlock_irqrestore(&ftsm_data->irq_lock, irqflags);
    FTS_FUNC_EXIT();
}

void ftsm_irq_enable(void)
{
    ...
    if (ftsm_data->irq_disabled) {
        enable_irq(ftsm_data->irq);
        ftsm_data->irq_disabled = false;
    }
...
void ftsm_hid2std(void)
{
    int ret = 0;
    u8 buf[3] = {0xEB, 0xAA, 0x09};

    if (ftsm_data->bus_type != BUS_TYPE_I2C)
        return;

    ret = ftsm_write(buf, 3);
    if (ret < 0) {
        FTS_ERROR("hid2std cmd write fail");
    } else {
        msleep(10);
        buf[0] = buf[1] = buf[2] = 0;
        ret = ftsm_read(NULL, 0, buf, 3);
        if (ret < 0) {
...
static int fts_read_bootid(struct fts_ts_data *ts_data, u8 *id)
{
    ...
    ret = ftsm_read(id_cmd, id_cmd_len, chip_id, 2);
    if ((ret < 0) || (0x0 == chip_id[0]) || (0x0 == chip_id[1])) {
        FTS_ERROR("read ftsm boot id fail,read:0x%02x%02x", chip_id[0], chip_id[1]);
        return -EIO;
    }
...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    do {
        ret = ftsm_read_reg(FTS_REG_CHIP_ID, &chip_id[0]);
        ret = ftsm_read_reg(FTS_REG_CHIP_ID2, &chip_id[1]);
	FTS_DEBUG("chip id read invalid info, read:0x%02x%02x ret = %d",
                 chip_id[0], chip_id[1], ret);
        if ((ret < 0) || (0x0 == chip_id[0]) || (0x0 == chip_id[1])) {
            FTS_DEBUG("chip id read ftsm invalid, read:0x%02x%02x",
                      chip_id[0], chip_id[1]);
        } else {
            ...
            if (!ret)
                ...
                FTS_DEBUG("TP not ready ftsm, read:0x%02x%02x",
                          chip_id[0], chip_id[1]);
        }
    ...
    if ((cnt * INTERVAL_READ_REG) >= TIMEOUT_READ_REG) {
        FTS_INFO("fw is invalid, need read boot id ftsm");
        ...
        if (ret <  0) {
            FTS_ERROR("read boot id ftsm fail");
            return ret;
        }
...
void ftsm_release_all_finger(void)
{
    struct fts_ts_data *ts_data = ftsm_data;
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    if (data->gesture_mode) {
        if (0 == ftsm_gesture_readdata(data, NULL)) {
    ...
    ret = ftsm_read(buf, 1, buf + 1, size);
    if (ret < 0) {
    ...
    if ((buf[size] != 0xFF) && (size < (data->pnt_buf_size - 1))
        ...
        buf[0] += size;
        ret = ftsm_read(buf, 1, buf + 1 + size, data->pnt_buf_size - 1 - size);
        if (ret < 0) {
...
static int fts_read_parse_touchdata(struct fts_ts_data *data)
{
    ...
    if (data->ic_info.is_incell) {
        if ((data->point_num == 0x0F) && (buf[2] == 0xFF) && (buf[3] == 0xFF)
            ...
            ftsm_release_all_finger();
            ftsm_tp_state_recovery(data);
            data->point_num = 0;
            return -EIO;
        }
    }
...
#if FTS_PSENSOR_EN
        if (fts_proximity_readdata(ftsm_data) == 0)
            continue;
#endif
...
static int fts_irq_registration(struct fts_ts_data *ts_data)
{
    ...
    FTS_INFO("set key capabilities fts_irq_registration \n");
...
static int fts_input_init(struct fts_ts_data *ts_data)
{
    ...
    FTS_INFO("set key capabilities fts_input_init \n");
...
extern int ftsm_open_short_test(char *ini_file_name);
u8 focalm_lcm_maker;
...
static int fts_set_ini_name(char *cfgname)
{
    ...
    if(vendor_id == BOE_VENDOR){
        ...
        sprintf(cfgname, "%s", "FTSM_FOR_BOE.ini");
		printk("-----fts_set_ini_name ft8201m----------------%s\n");
    }
	...
	//if(vendor_id == 0x02){
	//	printk("---------------fts_set_ini_name---------\n");
    ///    sprintf(cfgname, "%s", "FTSM_FOR_BOE.ini");
	//	printk("---------------fts_set_ini_name-------- = %s\n", cfgname);
    //}
...
static ssize_t ctp_open_proc_read(struct file *file, char __user *buf,size_t count, loff_t *ppos)
{
    ...
    *ppos += count;
    ...
    ret = ftsm_open_short_test(fwname);
    if(ret == 1){
        result = 1;
...
void create_ctpm_proc(void)
{
    ...
    printk("openshort create_ctpm_proc \n");
...
printk("[FTS_TS]%s: ftsm Enter\n", __func__);
    TPD_DMESG("touch driver is ftsm_ts_probe_entry %s \n", __LINE__);
    FTS_FUNC_ENTER();
    ...
    TPD_DMESG("touch driver is fts_tsm_probe_entry %s \n", __LINE__);
    spin_lock_init(&ts_data->irq_lock);
    mutex_init(&ts_data->report_mutex);
    mutex_init(&ts_data->bus_lock);

    /* Init communication interface */
    ret = ftsm_bus_init(ts_data);
    if (ret) {
    ...
    if (ret) {
        FTS_ERROR("not focal IC, unregister driver ftsm");
        goto err_irq_req;
    }

    ret = ftsm_create_apk_debug_channel(ts_data);
    if (ret) {
        FTS_ERROR("create apk debug node fail");
    }

    ret = ftsm_create_sysfs(ts_data);
    if (ret) {
    ...
    ret = ftsm_ex_mode_init(ts_data);
    if (ret) {
        FTS_ERROR("init glove/cover/charger fail");
    }

    ret = ftsm_gesture_init(ts_data);
    if (ret) {
...
static int fts_ts_remove_entry(struct fts_ts_data *ts_data)
{
    ...
    ftsm_release_apk_debug_channel(ts_data);
    ftsm_remove_sysfs(ts_data);
    ftsm_ex_mode_exit(ts_data);

    ftsm_fwupg_exit(ts_data);

#if FTS_TEST_EN
    ftsm_test_exit(ts_data);
#endif
    ...
    ftsm_gesture_exit(ts_data);
    ftsm_bus_exit(ts_data);
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("[FTS_TS]%s: ftsm Enter\n", __func__);
    FTS_INFO("Touch Screen(I2C BUS) driver prboe ftsm_ts_probe ftsm_ts...\n");
    ...
    if (!ts_data) {
        FTS_ERROR("allocate memory for ftsm_data fail");
        return -ENOMEM;
    }
...
{
    FTS_INFO("Touch Screen(I2C BUS) driver prboe ftsm_ts_driver_init ftsm_ts...\n");
...
static int tpd_local_init(void)
{
    int ret = 0;
	 printk("[FTS_TS]%s: ftsm Enter\n", __func__);
	printk("Touch Screen(I2C BUS) driver ptpd_local_init ftsm_ts_driver_init ftsm_ts...\n" );
    FTS_FUNC_ENTER();
...
static void tpd_suspend(struct device *dev)
{
    ...
    if (ts_data->gesture_mode) {
        ftsm_gesture_suspend(ts_data);
    } else {
        ftsm_irq_disable();

        FTS_INFO("make TP enter into sleep mode");
        ret = ftsm_write_reg(FTS_REG_POWER_MODE, FTS_REG_POWER_MODE_SLEEP);
        if (ret < 0)
...
static int __init tpd_driver_init(void)
{
    FTS_FUNC_ENTER();
	printk("[FTS_TS]%s: ftsm Enter\n", __func__);
    FTS_INFO("Driver version ftsm : %s", FTS_DRIVER_VERSION);
    tpd_get_dts_info();
