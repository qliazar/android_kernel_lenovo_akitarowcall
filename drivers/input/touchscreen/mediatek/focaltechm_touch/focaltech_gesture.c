    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mtp_gesture_mode = true;
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        FTS_DEBUG("disable gesture");
        ts_data->gesture_mode = DISABLE;
	mtp_gesture_mode = false;
    }
    mutex_unlock(&ts_data->input_dev->mutex);
...
int ftsm_gesture_readdata(struct fts_ts_data *ts_data, u8 *data)
{
    ...
    ret = ftsm_read_reg(FTS_REG_GESTURE_EN, &buf[0]);
    ...
    ret = ftsm_read(&buf[2], 1, &buf[2], FTS_GESTURE_DATA_LEN - 2);
    if (ret < 0) {
...
void ftsm_gesture_recovery(struct fts_ts_data *ts_data)
{
    if (ts_data->gesture_mode && ts_data->suspended) {
        ...
        ftsm_write_reg(0xD1, 0xFF);
        ftsm_write_reg(0xD2, 0xFF);
        ftsm_write_reg(0xD5, 0xFF);
        ftsm_write_reg(0xD6, 0xFF);
        ftsm_write_reg(0xD7, 0xFF);
        ftsm_write_reg(0xD8, 0xFF);
        ftsm_write_reg(FTS_REG_GESTURE_EN, ENABLE);
    }
}
...
int ftsm_gesture_suspend(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        ftsm_write_reg(FTS_REG_GESTURE_EN, ENABLE);
        msleep(1);
        ftsm_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state == ENABLE)
            break;
    }
...
int ftsm_gesture_resume(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 5; i++) {
        ftsm_write_reg(FTS_REG_GESTURE_EN, DISABLE);
        msleep(1);
        ftsm_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state == DISABLE)
            break;
