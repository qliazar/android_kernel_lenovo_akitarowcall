...
struct upgrade_module modulem_list[] = {
    {FTS_MODULE_ID, FTS_MODULE_NAME, fwm_file, sizeof(fwm_file)},
    {FTS_MODULE2_ID, FTS_MODULE2_NAME, fwm_file2, sizeof(fwm_file2)},
    {FTS_MODULE3_ID, FTS_MODULE3_NAME, fwm_file3, sizeof(fwm_file3)},
};

struct upgrade_func *upgradem_func_list[] = {
    &upgrade_func_ft8201,
    &upgrade_func_ft8201ab,
};

struct fts_upgrade *fwupgradem;
...
static int fts_fwupg_reset_to_boot(struct fts_upgrade *upg)
{
    ...
    ret = ftsm_write_reg(reg, FTS_UPGRADE_AA);
    if (ret < 0) {
    ...
    ret = ftsm_write_reg(reg, FTS_UPGRADE_55);
    if (ret < 0) {
    ...
    u32 ecc_length)
{
    ...
    ret = ftsm_write(cmd, FTS_ROMBOOT_CMD_ECC_NEW_LEN);
    if (ret < 0) {
...
static int fts_pram_ecc_cal_xor(void)
{
    ...
    ret = ftsm_read_reg(FTS_ROMBOOT_CMD_ECC, &reg_val);
    if (ret < 0) {
        FTS_ERROR("read pramboot ecc fail");
        return ret;
    }
...
static int fts_pram_write_buf(struct fts_upgrade *upg, u8 *buf, u32 len)
{
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        if (upg->ts_data->bus_type == BUS_TYPE_SPI_V2) {
            ...
            ret = ftsm_write(packet_buf, FTS_ROMBOOT_CMD_SET_PRAM_ADDR_LEN);
            if (ret < 0) {
        ...
        ret = ftsm_write(packet_buf, packet_len + cmdlen);
        if (ret < 0) {
...
static int fts_pram_init(void)
{
    ...
    ret = ftsm_read(wbuf, 1, &reg_val, 1);
    if (ret < 0) {
...
/************************************************************************
* Name: ftsm_fwupg_enter_into_boot
* Brief: enter into boot environment, ready for upgrade
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int ftsm_fwupg_enter_into_boot(void)
{
    int ret = 0;
    bool fwvalid = false;
    bool state = false;
    struct fts_upgrade *upg = fwupgradem;
    ...
    int retries_delay)
{
    ...
    for (i = 0; i < retries; i++) {
        ...
        ret = ftsm_read(&cmd , 1, val, FTS_CMD_FLASH_STATUS_LEN);
...
/************************************************************************
 * Name: ftsm_fwupg_erase
 * Brief: erase flash area
 * Input: delay - delay after erase
 * Output:
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int ftsm_fwupg_erase(u32 delay)
{
    int ret = 0;
    u8 cmd = 0;
    bool flag = false;
 ...
 ***********************************************************************/
int ftsm_fwupg_ecc_cal(u32 saddr, u32 len)
{
    int ret = 0;
    u32 i = 0;
    ...
    ret = ftsm_read(wbuf, 1, val, ecc_len);
    if (ret < 0) {
    ...
    u32 delay)
{
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        if (upg->ts_data->bus_type == BUS_TYPE_SPI_V2) {
            ...
            ret = ftsm_write(packet_buf, FTS_LEN_SET_ADDR);
            if (ret < 0) {
 ...
 ***********************************************************************/
int ftsm_flash_read_buf(u32 saddr, u8 *buf, u32 len)
{
    int ret = 0;
    u32 i = 0;
    u32 packet_number = 0;
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        if (upg->ts_data->bus_type == BUS_TYPE_I2C) {
            ...
            wbuf[2] = BYTE_OFF_8(addr);
            wbuf[3] = BYTE_OFF_0(addr);
            ret = ftsm_write(wbuf, FTS_CMD_READ_LEN);
            if (ret < 0) {
            ...
            ret = ftsm_read(NULL, 0, buf + offset, packet_len);
            if (ret < 0) {
            ...
            wbuf[2] = BYTE_OFF_8(addr);
            wbuf[3] = BYTE_OFF_0(addr);
            ret = ftsm_write(wbuf, FTS_LEN_SET_ADDR);
            if (ret < 0) {
                FTS_ERROR("set flash address fail");
                return ret;
            }
            ...
            wbuf[0] = FTS_CMD_READ;
            ret = ftsm_read(wbuf, 1, buf + offset, packet_len);
            if (ret < 0) {
            ...
            wbuf[4] = BYTE_OFF_8(packet_len);
            wbuf[5] = BYTE_OFF_0(packet_len);
            ret = ftsm_read(wbuf, FTS_CMD_READ_LEN_SPI, \
...
static int fts_flash_read(u32 addr, u8 *buf, u32 len)
{
    ...
    ret = ftsm_fwupg_enter_into_boot();
    if (ret < 0) {
        FTS_ERROR("enter into pramboot/bootloader fail");
        goto read_flash_err;
    }

    ret = ftsm_flash_read_buf(addr, buf, len);
    if (ret < 0) {
...
int ftsm_upgrade_bin(char *fw_name, bool force)
{
    int ret = 0;
    u32 fw_file_len = 0;
    u8 *fw_file_buf = NULL;
...
int ftsm_enter_test_environment(bool test_state)
{
    return 0;
}
#if FTS_AUTO_LIC_UPGRADE_EN
static int fts_lic_get_vid_in_tp(u16 *vid)
{
    ...
    ret = ftsm_read_reg(FTS_REG_VENDOR_ID, &val[0]);
    if (ftsm_data->ic_info.is_incell)
        ret = ftsm_read_reg(FTS_REG_MODULE_ID, &val[1]);
    if (ret < 0) {
...
static int fts_lic_get_ver_in_tp(u8 *ver)
{
    ...
    ret = ftsm_read_reg(FTS_REG_LIC_VER, ver);
    if (ret < 0) {
...
static int fts_param_get_ver_in_tp(u8 *ver)
{
    ...
    ret = ftsm_read_reg(FTS_REG_IDE_PARA_VER_ID, ver);
    if (ret < 0) {
...
static int fts_param_get_ver_in_host(struct fts_upgrade *upg, u8 *ver)
{
    if ((!upg) || (!upg->func) || (!upg->fw) || (!ver)) {
        FTS_ERROR("ftsm_data/upgrade/func/fw/ver is NULL");
        return -EINVAL;
    }
...
static int fts_param_ide_in_host(struct fts_upgrade *upg)
{
    ...
    if ((!upg) || (!upg->func) || (!upg->fw)) {
        FTS_ERROR("ftsm_data/upgrade/func/fw is NULL");
        return -EINVAL;
    }
...
static int fts_param_ide_in_tp(u8 *val)
{
    ...
    ret = ftsm_read_reg(FTS_REG_IDE_PARA_STATUS, val);
    if (ret < 0) {
...
static int fts_fwupg_get_ver_in_tp(u8 *ver)
{
    ...
    ret = ftsm_read_reg(FTS_REG_FW_VER, ver);
    if (ret < 0) {
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int ftsm_fwupg_upgrade(struct fts_upgrade *upg)
{
    int ret = 0;
...
static int fts_fwupg_get_vendorid(struct fts_upgrade *upg, int *vid)
{
    ...
    fwvalid = fts_fwupg_check_fw_valid();
    if (fwvalid) {
        ret = ftsm_read_reg(FTS_REG_VENDOR_ID, &vendor_id);
        if (upg->ts_data->ic_info.is_incell)
            ret = ftsm_read_reg(FTS_REG_MODULE_ID, &module_id);
    } else {
...
static int fts_fwupg_get_module_info(struct fts_upgrade *upg)
{
    int ret = 0;
    int i = 0;
    struct upgrade_module *info = &modulem_list[0];
...
{
    upg->fw = upg->module_info->fwm_file;
...
void hardwareinfom_set(void)
{
    ...
    ftsm_read_reg(FTS_REG_FW_VER, &fw_ver);
    ftsm_read_reg(FTS_REG_VENDOR_ID, &vendor_id);
    ftsm_read_reg(FTS_REG_CHIP_ID, &ic_type);
	...
	else if(vendor_id== BOEM_VENDOR)
    {
        snprintf(vendor_for_id,HARDWARE_MAX_ITEM_LONGTH,"BOEM");
		printk("------BOEM----------------------\n");
    }
    ...
    if(ic_type == TP_IC_FT8201)
    {
        snprintf(ic_name,HARDWARE_MAX_ITEM_LONGTH,"FT8201M");
		printk("------FT8201M----------------------\n");
    } 
	else{
...
static void fts_fwupg_work(struct work_struct *work)
{
    ...
    hardwareinfom_set();//type,liutongxing.wt,add,20190523,tp update fw display hardwareinfo.

    ftsm_irq_enable();
    upg->ts_data->fw_loading = 0;
}

int ftsm_fwupg_init(struct fts_ts_data *ts_data)
{
    int i = 0;
    int j = 0;
    u16 ic_stype = 0;
    struct upgrade_func *func = upgradem_func_list[0];
    int func_count = sizeof(upgradem_func_list) / sizeof(upgradem_func_list[0]);
    ...
    fwupgradem = (struct fts_upgrade *)kzalloc(sizeof(*fwupgradem), GFP_KERNEL);
    if (NULL == fwupgradem) {
    ...
    if (1 == func_count) {
        fwupgradem->func = func;
    } else {
        for (i = 0; i < func_count; i++) {
            func = upgradem_func_list[i];
...
int ftsm_fwupg_exit(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    if (fwupgradem) {
        if (fwupgradem->fw_from_request) {
            vfree(fwupgradem->fw);
            fwupgradem->fw = NULL;
        }
