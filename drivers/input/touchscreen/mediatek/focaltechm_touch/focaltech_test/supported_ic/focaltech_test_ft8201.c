...
{
    if (ftsm_data->client->addr != info->m_i2c_addr) {
        FTS_TEST_DBG("change i2c addr to master(0x%x)\n", info->m_i2c_addr);
        ftsm_data->client->addr = info->m_i2c_addr;
    }
}

static void work_as_slave(struct ft8201_info *info)
{
    if (ftsm_data->client->addr != info->s_i2c_addr) {
        FTS_TEST_DBG("change i2c addr to slave(0x%x)\n", info->s_i2c_addr);
        ftsm_data->client->addr = info->s_i2c_addr;
    }
}
...
static int ft8201_write_reg(struct ft8201_info *info, u8 reg_addr, u8 reg_val)
{
    ...
    if (chip_have_two_heart(info)) {
        /* write master reg */
        work_as_master(info);
        ret = ftsm_test_write_reg(reg_addr, reg_val);
        if (ret) {
...
static int get_chip_information(struct ft8201_info *info)
{
    ...
    ret = ftsm_test_read_reg(REG_MS_SELECT, &value[0]);
    if (ret) {
    ...
    ret = ftsm_test_read_reg(REG_CH_X_MASTER, &value[0]);
    if (ret) {
    ...
    ret = ftsm_test_read_reg(REG_CH_Y_MASTER, &value[0]);
    if (ret) {
    ...
    ret = ftsm_test_read_reg(REG_CH_X_SLAVE, &value[0]);
    if (ret) {
    ...
    ret = ftsm_test_read_reg(REG_CH_Y_SLAVE, &value[0]);
    if (ret) {
    ...
    ret = ftsm_test_write_reg(REG_FW_INFO_CNT, 0);
    if (ret) {
        FTS_TEST_SAVE_ERR("write fw into cnt fail\n");
        return ret;
    }
    cmd = REG_FW_INFO_ADDR;
    ret = ftsm_test_read(cmd, &value[0], REG_FW_INFO_LEN);
    if (ret) {
...
static int ft8201_get_tx_rx_cb(struct ft8201_info *info, u8 start_node, int read_num, int *read_buffer)
{
    ...
    FTS_TEST_FUNC_ENTER();
    if (chip_have_two_heart(info)) {
        buffer_master = ftsm_malloc((master_tx + 1) * master_rx * sizeof(int));
        ...
        buffer_slave = ftsm_malloc((slave_tx + 1) * slave_rx * sizeof(int));
        ...
        /* master cb */
        work_as_master(info);
        ret = getm_cb_incell(0, master_tx * master_rx  + 6, buffer_master);
        if (ret ) {
        ...
        /* slave cb */
        work_as_slave(info);
        ret = getm_cb_incell(0, slave_tx * slave_rx + 6, buffer_slave);
        if (ret ) {
...
GET_CB_ERR:
        ...
        ret = getm_cb_incell(0, read_num, read_buffer);
        if (ret ) {
...
static int get_order_incell(u16 saddr, int byte_num, int *cb_buf)
{
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        ret = ftsm_test_write_reg(FACTORY_REG_ORDER_ADDR_H, addr_h);
        if (ret) {
            FTS_TEST_SAVE_ERR("write order addr high fail\n");
            goto TEST_CB_ERR;
        }
        ret = ftsm_test_write_reg(FACTORY_REG_ORDER_ADDR_L, addr_l);
        if (ret) {
...
static int ft8201_get_tx_rx_order(struct ft8201_info *info, int read_num, int *read_buffer)
{
    ...
    FTS_TEST_FUNC_ENTER();
    if (chip_have_two_heart(info)) {
        buffer_master = ftsm_malloc((master_tx + 1) * master_rx * 2 * sizeof(int));
        if (NULL == buffer_master) {
            FTS_TEST_SAVE_ERR("%s:master buf malloc fail\n", __func__);
            ret = -ENOMEM;
            goto GET_ORDER_ERR;
        ...
        buffer_slave = ftsm_malloc((slave_tx + 1) * slave_rx * 2 * sizeof(int));
        if (NULL == buffer_slave) {
            FTS_TEST_SAVE_ERR("%s:slave buf malloc fail\n", __func__);
            ret = -ENOMEM;
            goto GET_ORDER_ERR;
                                    ...
                                    u8 reg20_val, u8 reg2e_val, u8 data_type, int *data)
{
    ...
    ret = waitm_state_update(TEST_RETVAL_00);
    if (ret < 0) {
...
static int ft8201_short_test(struct ft8201_info *info, struct fts_test *tdata, bool *test_result)
{
    ...
    /* save */
    showm_data(adcdata, true);

    /* compare */
    tmp_result = comparem_data(adcdata, thr->basic.short_res_min, TEST_SHORT_RES_MAX, thr->basic.short_res_vk_min, TEST_SHORT_RES_MAX, true);
...
test_err:
    ...
    /*save test data*/
    ftsm_test_save_data("Short Circuit Test", CODE2_SHORT_TEST,
...
static int ft8201_open_test(struct ft8201_info *info, struct fts_test *tdata, bool *test_result)
{
    ...
    ret = ftsm_test_read_reg(FACTORY_REG_OPEN_REG86, &reg86_val);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("read 0x86 fail\n");
        goto test_err;
    }

    ret = ftsm_test_read_reg(FACTORY_REG_OPEN_REG20, &reg20_val);
    if (ret < 0) {
    ...
    /* show open data */
    showm_data(opendata, false);

    /* compare */
    min = thr->basic.open_cb_min;
    max = 256;
    FTS_TEST_DBG("open %d %d\n", min, opendata[0]);
    tmp_result = comparem_data(opendata, min, max, 0, 0, false);
...
test_err:
    ...
    /* save data */
    ftsm_test_save_data("Open Test", CODE_OPEN_TEST,
...
static int ft8201_mux_open_test(struct ft8201_info *info, struct fts_test *tdata, bool *test_result)
{
    ...
    ret = ftsm_test_read_reg(FACTORY_REG_OPEN_REG2E, &reg2e_val);
    if (ret < 0) {
    ...
    showm_data(mux_open_data_tmp, false);

    /* compare */
    tmp_result = comparem_array(mux_open_data_tmp, mux2_open_min, mux2_open_max, false);
    ...
    tmp_result = comparem_array(mux_open_data_tmp, mux1_open_min, mux1_open_max, false);
...
test_err:
    ...
    /* save data */
    ftsm_test_save_data("Mux Open Test", CODE_MUX_OPEN_TEST,
...
static int ft8201_cb_test(struct ft8201_info *info, struct fts_test *tdata, bool *test_result)
{
    ...
    showm_data(cbdata, key_check);

    /* compare */
    tmp_result = comparem_array(cbdata, thr->cb_min, thr->cb_max, key_check);
...
test_err:
    ...
    /*save test data*/
    ftsm_test_save_data("CB Test", CODE_CB_TEST,
...
static int ft8201_rawdata_test(struct ft8201_info *info, struct fts_test *tdata, bool *test_result)
{
    ...
    /* save */
    showm_data(rawdata, key_check);

    /* compare */
    tmp_result = comparem_array(rawdata,
                               thr->rawdata_min,
                               thr->rawdata_max,
                               key_check);
...
test_err:
    ...
    /*save test data*/
    ftsm_test_save_data("RawData Test", CODE_RAWDATA_TEST,
...
static int ft8201_lcdnoise_test(struct ft8201_info *info, struct fts_test *tdata, bool *test_result)
{
    ...
    ret = ftsm_test_read_reg(FACTORY_REG_DATA_SELECT, &old_mode);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("read reg06 fail\n");
        goto test_err;
    }

    ret = ftsm_test_read_reg(FACTORY_NOISE_MODE_REG, &reg_value);
    if (ret < 0) {
    ...
    /* read noise data */
    ret = ftsm_test_write_reg(FACTORY_REG_LINE_ADDR, 0xAD);
    if (ret < 0) {
    ...
    sysm_delay(frame_num * FACTORY_TEST_DELAY / 2);
    for (i = 0; i < FACTORY_TEST_RETRY; i++) {
        status = 0xFF;
        ret = ftsm_test_read_reg(FACTORY_REG_LCD_NOISE_START, &status );
    ...
    byte_num = tdata->node.node_num * 2;
    ret = readm_mass_data(FACTORY_REG_RAWDATA_ADDR, byte_num, lcdnoise);
    if (ret < 0) {
    ...
    tmp_result = comparem_data(lcdnoise, 0, max, 0, max_vk, true);

    ret = 0;

restore_reg:
    ret = ftsm_test_write_reg(FACTORY_REG_DATA_SELECT, old_mode);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("restore reg06 fail\n");
    }
    ret = ftsm_test_write_reg(FACTORY_NOISE_MODE_REG, reg_value);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("restore reg5e fail\n");
    }
    ret = ftsm_test_write_reg(FACTORY_REG_LCD_NOISE_START, 0x00);
    if (ret < 0) {
...
test_err:
    ...
    /*save test data*/
    ftsm_test_save_data("LCD Noise Test", CODE2_LCD_NOISE_TEST,
...
static int start_test_ft8201(void)
{
    ...
    FTS_TEST_FUNC_ENTER();
    FTS_TEST_INFO("test item:0x%x", ftsm_ftest->ic.incell.u.tmp);
...
{
    struct incell_threshold *thr = &ftsm_ftest->ic.incell.thr;
...
struct test_funcs testm_func_ft8201 = {
    .ctype = {0x10, 0x1A},
    .hwtype = IC_HW_INCELL,
    .key_num_total = 12,
