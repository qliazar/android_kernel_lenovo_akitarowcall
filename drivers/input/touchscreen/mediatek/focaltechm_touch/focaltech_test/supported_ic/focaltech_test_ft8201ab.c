...
static int ft8201ab_short_test(struct fts_test *tdata, bool *test_result)
{
    ...
    ret = shortm_get_adcdata_incell(TEST_RETVAL_AA, ch_num, byte_num, adcdata);
    if (ret < 0) {
    ...
    /* save */
    showm_data(adcdata, false);

    /* compare */
    min = thr->basic.short_res_min;
    max = TEST_SHORT_RES_MAX;
    tmp_result = comparem_data(adcdata, min, max, min, max, false);

    ret = 0;
test_err:
    ret = ftsm_test_write_reg(FACTORY_REG_SHORT_TEST_STATE, 0x03);
    if (ret < 0) {
    ...
    /* save test data */
    ftsm_test_save_data("Short Circuit Test", CODE_SHORT_TEST,
...
static int ft8201ab_open_test(struct fts_test *tdata, bool *test_result)
{
    ...
    ret = ftsm_test_write_reg(FACTORY_REG_OPEN_START, 0x01);
    if (ret < 0) {
    ...
    for (i = 0; i < FACTORY_TEST_RETRY; i++) {
        ...
        ret = ftsm_test_read_reg(FACTORY_REG_OPEN_STATE, &state);
    ...
    /* get open diff data */
    byte_num = tdata->node.node_num * 2;
    ret = readm_mass_data(FACTORY_REG_OPEN_ADDR, byte_num, opendata);
    if (ret) {
...
test_err:
    ret = ftsm_test_write_reg(FACTORY_REG_OPEN_STATE, 0x03);
    if (ret < 0) {
...
static int ft8201ab_cb_test(struct fts_test *tdata, bool *test_result)
{
    ...
    /* cb test enable */
    ret = ftsm_test_write_reg(FACTORY_REG_CB_TEST_EN, 0x01);
    if (ret < 0) {
    ...
    ret = getm_cb_incell(0, byte_num, cbdata);
    if (ret < 0) {
    ...
    /* save */
    showm_data(cbdata, false);

    /* compare */
    tmp_result = comparem_array(cbdata, thr->cb_min, thr->cb_max, false);

test_err:
    /* cb test disable */
    ret = ftsm_test_write_reg(FACTORY_REG_CB_TEST_EN, 0x00);
    if (ret < 0) {
...
static int ft8201ab_rawdata_test(struct fts_test *tdata, bool *test_result)
{
    ...
    /* rawdata test enable */
    ret = ftsm_test_write_reg(FACTORY_REG_RAWDATA_TEST_EN, 0x01);
    if (ret < 0) {
...
test_err:
    /* rawdata test disble */
    ret = ftsm_test_write_reg(FACTORY_REG_RAWDATA_TEST_EN, 0x00);
    if (ret < 0) {
...
static int ft8201ab_lcdnoise_test(struct fts_test *tdata, bool *test_result)
{
    ...
    ret =  ftsm_test_write_reg(FACTORY_REG_DATA_SELECT, 0x01);
    if (ret < 0) {
    ...
    ret = ftsm_test_write_reg(FACTORY_REG_LCD_NOISE_FRAME, frame_num / 4);
    if (ret < 0) {
    ...
    /* start test */
    ret = ftsm_test_write_reg(FACTORY_REG_LCD_NOISE_START, 0x01);
    if (ret < 0) {
    ...
    for (i = 0; i < FACTORY_TEST_RETRY; i++) {
        status = 0xFF;
        ret = ftsm_test_read_reg(FACTORY_REG_LCD_NOISE_TEST_STATE, &status);
...
test_err:
    ...
    ret = ftsm_test_write_reg(FACTORY_REG_LCD_NOISE_TEST_STATE, 0x03);
    if (ret < 0) {
    ...
    /*save test data*/
    ftsm_test_save_data("LCD Noise Test", CODE_LCD_NOISE_TEST,
...
struct test_funcs testm_func_ft8201ab = {
    .ctype = {0x21},
    .hwtype = IC_HW_INCELL,
    .key_num_total = 0,
