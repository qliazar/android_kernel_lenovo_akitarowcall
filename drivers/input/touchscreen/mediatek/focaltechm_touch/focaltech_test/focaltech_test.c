...
*****************************************************************************/
struct fts_test *ftsm_ftest;

struct test_funcs *testm_func_list[] = {
    &testm_func_ft8201,
    &testm_func_ft8201ab,
};
...
void *ftsm_malloc(size_t size)
{
    return kzalloc(size, GFP_KERNEL);
}

void ftsm_free_proc(void *p)
{
    return kfree(p);
}

void printm_buffer(int *buffer, int length, int line_num)
{
    int i = 0;
    int j = 0;
    int tmpline = 0;
    ...
    int tmplen = 0;
    int cnt = 0;
    struct fts_test *tdata = ftsm_ftest;
    ...
    u8 *cmd, int cmdlen, u8 *data, int datalen)
{
    ...
    ret = ftsm_read(cmd, cmdlen, data, datalen);
    if (ret < 0)
        return ret;
    else
        return 0;
...
static int fts_test_bus_write(u8 *writebuf, int writelen)
{
    ...
    ret = ftsm_write(writebuf, writelen);
    if (ret < 0)
        return ret;
    else
        return 0;
...
int ftsm_test_read_reg(u8 addr, u8 *val)
{
    return fts_test_bus_read(&addr, 1, val, 1);
}

int ftsm_test_write_reg(u8 addr, u8 val)
{
    int ret;
    u8 cmd[2] = {0};
...
int ftsm_test_read(u8 addr, u8 *readbuf, int readlen)
{
    int ret = 0;
    int i = 0;
    int packet_length = 0;
...
int ftsm_test_write(u8 addr, u8 *writebuf, int writelen)
{
    int ret = 0;
    int i = 0;
    u8 *data = NULL;
    ...
    data = ftsm_malloc(BYTES_PER_TIME + 1);
    if (!data) {
...
int enterm_work_mode(void)
{
    ...
    ret = ftsm_test_read_reg(DEVIDE_MODE_ADDR, &mode);
    if ((ret >= 0) && (0x00 == mode))
        return 0;

    for (i = 0; i < ENTER_WORK_FACTORY_RETRIES; i++) {
        ret = ftsm_test_write_reg(DEVIDE_MODE_ADDR, 0x00);
        if (ret >= 0) {
            sysm_delay(FACTORY_TEST_DELAY);
            for (j = 0; j < 20; j++) {
                ret = ftsm_test_read_reg(DEVIDE_MODE_ADDR, &mode);
                if ((ret >= 0) && (0x00 == mode)) {
...
int enterm_factory_mode(void)
{
    ...
    for (i = 0; i < ENTER_WORK_FACTORY_RETRIES; i++) {
        ret = ftsm_test_write_reg(DEVIDE_MODE_ADDR, 0x40);
        if (ret >= 0) {
            sysm_delay(FACTORY_TEST_DELAY);
            for (j = 0; j < 20; j++) {
                ret = ftsm_test_read_reg(DEVIDE_MODE_ADDR, &mode);
...
/*
 * readm_mass_data - read rawdata/short test data
 ...
 */
int readm_mass_data(u8 addr, int byte_num, int *buf)
{
    int ret = 0;
    int i = 0;
    u8 *data = NULL;

    data = (u8 *)ftsm_malloc(byte_num * sizeof(u8));
    if (NULL == data) {
    ...
    ret = ftsm_test_read(addr, data, byte_num);
    if (ret < 0) {
...
int shortm_get_adcdata_incell(u8 retval, u8 ch_num, int byte_num, int *adc_buf)
{
    int ret = 0;
    int times = 0;
    u8 short_state = 0;
    ...
    /* Start ADC sample */
    ret = ftsm_test_write_reg(FACTORY_REG_SHORT_TEST_EN, 0x01);
    if (ret) {
        FTS_TEST_SAVE_ERR("start short test fail\n");
        goto adc_err;
    }

    sysm_delay(ch_num * FACTORY_TEST_DELAY);
    for (times = 0; times < FACTORY_TEST_RETRY; times++) {
        ret = ftsm_test_read_reg(FACTORY_REG_SHORT_TEST_STATE, &short_state);
        ...
        sysm_delay(FACTORY_TEST_RETRY_DELAY);
    }
    ...
    ret = readm_mass_data(FACTORY_REG_SHORT_ADDR, byte_num, adc_buf);
    if (ret) {
 ...
 */
int waitm_state_update(u8 retval)
{
    int ret = 0;
    int times = 0;
    u8 state = 0xFF;
    ...
    while (times++ < FACTORY_TEST_RETRY) {
        ...
        state = 0xFF;
        ret = ftsm_test_read_reg(FACTORY_REG_PARAM_UPDATE_STATE, &state);
...
/*
 * startm_scan - start to scan a frame
 */
int startm_scan(void)
{
    int ret = 0;
    u8 addr = 0;
    u8 val = 0;
    ...
    ret = ftsm_test_write_reg(addr, val);
    if (ret < 0) {
    ...
    while (times++ < FACTORY_TEST_RETRY) {
        ...
        ret = ftsm_test_read_reg(addr, &val);
    ...
    int *data)
{
    ...
    ret = ftsm_test_write_reg(off_addr, off_val);
    if (ret < 0) {
    ...
    if (tdata->func->raw_u16)
        ...
        ret = readm_mass_data(rawdata_addr, byte_num, data);
    if (ret < 0) {
...
int chipm_clb(void)
{
    ...
    /* start clb */
    ret = ftsm_test_write_reg(FACTORY_REG_CLB, 0x04);
    if (ret) {
    ...
    while (times++ < FACTORY_TEST_RETRY) {
        sysm_delay(FACTORY_TEST_RETRY_DELAY);
        ret = ftsm_test_read_reg(FACTORY_REG_CLB, &val);
        if ((0 == ret) && (0x02 == val)) {
            /* clb ok */
            break;
        } else
...
/*
 * getm_cb_incell - get cb data for incell IC
 */
int getm_cb_incell(u16 saddr, int byte_num, int *cb_buf)
{
    int ret = 0;
    int i = 0;
    u8 cb_addr = 0;
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        ret = ftsm_test_write_reg(FACTORY_REG_CB_ADDR_H, addr_h);
        if (ret) {
            FTS_TEST_SAVE_ERR("write cb addr high fail\n");
            goto TEST_CB_ERR;
        }
        ret = ftsm_test_write_reg(FACTORY_REG_CB_ADDR_L, addr_l);
        if (ret) {
            FTS_TEST_SAVE_ERR("write cb addr low fail\n");
            goto TEST_CB_ERR;
        }

        ret = ftsm_test_read(cb_addr, data + offset, read_num);
        if (ret) {
...
int getm_cb_sc(int byte_num, int *cb_buf, enum byte_mode mode)
{
    int ret = 0;
    int i = 0;
    int read_num = 0;
    ...
    cb = (u8 *)ftsm_malloc(byte_num * sizeof(u8));
    if (NULL == cb) {
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        ret = ftsm_test_write_reg(off_addr, offset);
        if (ret < 0) {
        ...
        if (tdata->func->cb_high_support) {
            ret = ftsm_test_write_reg(off_h_addr, offset >> 8);
            if (ret < 0) {
        ...
        ret = ftsm_test_read(cb_addr, cb + offset, read_num);
        if (ret < 0) {
            FTS_TEST_SAVE_ERR("read cb fail\n");
            goto cb_err;
        }
...
bool comparem_data(int *data, int min, int max, int min_vk, int max_vk, bool key)
{
    int i = 0;
    bool result = true;
    struct fts_test *tdata = ftsm_ftest;
...
bool comparem_array(int *data, int *min, int *max, bool key)
{
    int i = 0;
    bool result = true;
    struct fts_test *tdata = ftsm_ftest;
...
/*
 * showm_data - show and save test data to testresult.txt
 */
void showm_data(int *data, bool key)
{
#if TXT_SUPPORT
    int i = 0;
    int j = 0;
...
int mappingm_switch(u8 mapping)
{
    int ret = 0;
    u8 val = 0xFF;
    struct fts_test *tdata = ftsm_ftest;
    ...
    if (tdata->v3_pattern) {
        ret = ftsm_test_read_reg(FACTORY_REG_NOMAPPING, &val);
        if (ret < 0) {
        ...
        if (val != mapping) {
            ret = ftsm_test_write_reg(FACTORY_REG_NOMAPPING, mapping);
            if (ret < 0) {
...
bool getm_fw_wp(u8 wp_ch_sel, enum wp_type water_proof_type)
{
...
int getm_cb_mc_sc(u8 wp, int byte_num, int *cb_buf, enum byte_mode mode)
{
    int ret = 0;

    /* 1:waterproof 0:non-waterproof */
    ret = ftsm_test_write_reg(FACTORY_REG_MC_SC_MODE, wp);
    if (ret < 0) {
    ...
    /* read cb */
    ret = getm_cb_sc(byte_num, cb_buf, mode);
    if (ret < 0) {
...
int getm_rawdata_mc_sc(enum wp_type wp, int *data)
{
    int ret = 0;
    u8 val = 0;
    u8 addr = 0;
...
int getm_rawdata_mc(u8 fre, u8 fir, int *rawdata)
{
    int ret = 0;
    int i = 0;
    ...
    ret = ftsm_test_write_reg(FACTORY_REG_FRE_LIST, fre);
    if (ret < 0) {
    ...
    /* fir enable/disable */
    ret = ftsm_test_write_reg(FACTORY_REG_FIR, fir);
    if (ret < 0) {
...
int shortm_get_adc_data_mc(u8 retval, int byte_num, int *adc_buf, u8 mode)
{
    int ret = 0;
    int i = 0;
    u8 short_state = 0;
    ...
    ret = ftsm_test_write_reg(short_en_reg, mode);
    if (ret < 0) {
    ...
    for (i = 0; i < FACTORY_TEST_RETRY; i++) {
        ...
        ret = ftsm_test_read_reg(short_state_reg, &short_state);
        if ((ret >= 0) && (retval == short_state))
            break;
        else
    ...
    ret = readm_mass_data(short_data_reg, byte_num, adc_buf);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("get short(adc) data fail\n");
    }

    FTS_TEST_DBG("adc data:\n");
    printm_buffer(adc_buf, byte_num / 2, 0);
test_err:
    FTS_TEST_FUNC_EXIT();
    return ret;
}

bool comparem_mc_sc(bool tx_check, bool rx_check, int *data, int *min, int *max)
{
    int i = 0;
    bool result = true;
    struct fts_test *tdata = ftsm_ftest;
...
void showm_data_mc_sc(int *data)
{
    int i = 0;
    struct fts_test *tdata = ftsm_ftest;
...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
	...
	//sprintf(filepath, "%s%s", FTS_INI_FILE_PATH, file_name);//bug:446258,liutongxing.wt,remove,20190521,tp openshort test.
    sprintf(filepath, "%s%s", FTS_SAVE_OPENSHORT_TEST_DATA_FILE_PATH, file_name);//bug:446258,liutongxing.wt,add,20190521,tp openshort test.
    //snprintf(filepath, FILE_NAME_LENGTH, "%s%s", FTS_SAVE_OPENSHORT_TEST_DATA_FILE_PATH, file_name);
...
static int fts_test_get_item_count_scap_csv(int index)
{
    ...
    ret = ftsm_test_read_reg(FACTORY_REG_WC_SEL, &wc_sel);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("read water_channel_sel fail,ret=%d\n", ret);
        return index;
    }

    ret = ftsm_test_read_reg(FACTORY_REG_HC_SEL, &hc_sel);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("read high_channel_sel fail,ret=%d\n", ret);
        return index;
    }

    scap_select[0] = getm_fw_wp(wc_sel, WATER_PROOF_ON);
    scap_select[1] = getm_fw_wp(wc_sel, WATER_PROOF_OFF);
...
void ftsm_test_save_data_csv(struct fts_test *tdata)
{
#if CSV_SUPPORT
    int i = 0;
    int j = 0;
...
* Return:
*****************************************************************************/
void ftsm_test_save_data(char *name, int code, int *data, int datacnt,
                        bool mc_sc, bool key, bool result)
{
    ...
    info->data = ftsm_malloc(datalen * sizeof(int));
    if (!info->data) {
...
static int get_tx_rx_num(u8 tx_rx_reg, u8 *ch_num, u8 ch_num_max)
{
    ...
    for (i = 0; i < 3; i++) {
        ret = ftsm_test_read_reg(tx_rx_reg, ch_num);
...
static int get_key_num(int *key_num_en, int max_key_num)
{
    ...
    ret = ftsm_test_read_reg(FACTORY_REG_LEFT_KEY, &key_en);
    if (ret >= 0) {
        if (key_en & 0x01) {
            (*key_num_en)++;
        }
    ...
    ret = ftsm_test_read_reg(FACTORY_REG_RIGHT_KEY, &key_en);
    if (ret >= 0) {
        if (key_en & 0x01) {
            (*key_num_en)++;
        }
...
static int fts_test_init_basicinfo(struct fts_test *tdata)
{
    ...
    ftsm_test_read_reg(REG_FW_VERSION, &val);
    tdata->fw_ver = val;

    if (IC_HW_INCELL == tdata->func->hwtype) {
        ftsm_test_read_reg(REG_VA_TOUCH_THR, &val);
        tdata->va_touch_thr = val;
        ftsm_test_read_reg(REG_VKEY_TOUCH_THR, &val);
    ...
    if (IC_HW_MC_SC == tdata->func->hwtype) {
        ftsm_test_read_reg(FACTORY_REG_PATTERN, &val);
        tdata->v3_pattern = (1 == val) ? true : false;
        ftsm_test_read_reg(FACTORY_REG_NOMAPPING, &val);
        tdata->mapping = val;
    }
...
static int fts_test_main_init(void)
{
    ...
    tdata->buffer = (int *)ftsm_malloc(tdata->buffer_length);
...
static int fts_test_get_testparams(char *config_name)
{
    ...
    ret = ftsm_test_get_testparam_from_ini(config_name);
...
static int fts_test_entry(char *ini_file_name)
{
    ...
    if (true == fts_test_start()) {
        FTS_TEST_SAVE_INFO("\n\n=======Tp test pass.\n");
        //ftsm_ftest->result = true;
		ret = 1;
    } else {
        FTS_TEST_SAVE_INFO("\n\n=======Tp test failure.\n");
        //ftsm_ftest->result = false;
		ret = 0;
#if defined(TEST_SAVE_FAIL_RESULT) && TEST_SAVE_FAIL_RESULT
        do_gettimeofday(&(ftsm_ftest->tv));
#endif
    }
...
int ftsm_open_short_test(char *ini_file_name)
{
    return fts_test_entry( ini_file_name );
}
EXPORT_SYMBOL(ftsm_open_short_test);
#endif
//-bug:446258,liutongxing.wt,add,20190521,tp openshort test .
static ssize_t fts_test_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    struct fts_ts_data *ts_data = ftsm_data;
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    ret = ftsm_enter_test_environment(1);
    if (ret < 0) {
        FTS_ERROR("enter test environment fail");
    } else {
        fts_test_entry(fwname);
    }
    ret = ftsm_enter_test_environment(0);
    if (ret < 0) {
...
static int fts_test_func_init(struct fts_ts_data *ts_data)
{
    ...
    struct test_funcs *func = testm_func_list[0];
    int func_count = sizeof(testm_func_list) / sizeof(testm_func_list[0]);
    ...
    ftsm_ftest = (struct fts_test *)kzalloc(sizeof(*ftsm_ftest), GFP_KERNEL);
    if (NULL == ftsm_ftest) {
    ...
    for (i = 0; i < func_count; i++) {
        ...
        for (j = 0; j < FTS_MAX_COMPATIBLE_TYPE; j++) {
            if (0 == func->ctype[j])
                ...
                ftsm_ftest->func = func;
            }
        }
    }
    if (NULL == ftsm_ftest->func) {
        FTS_TEST_ERROR("no test function match, can't test");
        return -ENODATA;
    }

    ftsm_ftest->ts_data = ftsm_data;
    return 0;
}

int ftsm_test_init(struct fts_ts_data *ts_data)
{
    int ret = 0;
...
int ftsm_test_exit(struct fts_ts_data *ts_data)
{
    FTS_TEST_FUNC_ENTER();
