...
#define FTS_SAVE_OPENSHORT_TEST_DATA_FILE_PATH  "/mnt/sdcard/" 
#define false 0
#define true  1
...
*****************************************************************************/
extern struct test_funcs testm_func_ft8201;
extern struct test_funcs testm_func_ft8201ab;

extern struct fts_test *ftsm_ftest;
...
bool getm_fw_wp(u8 wp_channel_select, enum wp_type water_proof_type);
...
bool comparem_mc_sc(bool, bool, int *, int *, int *);
...
#define FTS_TEST_SAVE_INFO(fmt, args...) do { \
    if (ftsm_ftest->testresult) { \
        ftsm_ftest->testresult_len += snprintf( \
        ftsm_ftest->testresult + ftsm_ftest->testresult_len, \
        TXT_BUFFER_LEN, \
        fmt, ##args);\
    } \
} while (0)
...
#define FTS_TEST_SAVE_ERR(fmt, args...)  do { \
    if (ftsm_ftest->testresult && (ftsm_ftest->testresult_len < TXT_BUFFER_LEN)) { \
        ftsm_ftest->testresult_len += snprintf( \
        ftsm_ftest->testresult + ftsm_ftest->testresult_len, \
        TXT_BUFFER_LEN, \
        fmt, ##args);\
