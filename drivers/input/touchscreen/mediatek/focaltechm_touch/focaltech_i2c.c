...
static void fts_i2c_msg_dma_alloct(void)
{
    ...
    if (NULL == g_dma_buff_va) {
        ftsm_data->input_dev->dev.coherent_dma_mask = DMA_BIT_MASK(32);
        g_dma_buff_va = (u8 *)dma_alloc_coherent(&ftsm_data->input_dev->dev,
...
int ftsm_read(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
    ...
    if (!ts_data || !ts_data->client || !data || !datalen
        ...
        FTS_ERROR("ftsm_data/client/cmdlen(%d)/data/datalen(%d) is invalid",
                  cmdlen, datalen);
        return -EINVAL;
    }
...
int ftsm_write(u8 *writebuf, u32 writelen)
{
    ...
    if (!ts_data || !ts_data->client || !writebuf || !writelen
        ...
        FTS_ERROR("ftsm_data/client/data/datalen(%d) is invalid", writelen);
        return -EINVAL;
    }
    client = ts_data->client;
...
{
    return ftsm_read(&addr, 1, value, 1);
}
...
int ftsm_write_reg(u8 addr, u8 value)
{
    ...
    buf[0] = addr;
    buf[1] = value;
    return ftsm_write(buf, sizeof(buf));
}
