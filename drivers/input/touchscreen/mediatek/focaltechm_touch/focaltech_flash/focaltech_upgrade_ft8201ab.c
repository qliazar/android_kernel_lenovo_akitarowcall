...
static int fts_ft8201ab_upgrade_mode(enum FW_FLASH_MODE mode, u8 *buf, u32 len)
{
    ...
    ret = ftsm_write(cmd, FTS_CMD_DATA_LEN_LEN);
    if (ret < 0) {
    ...
    /* write app */
    ecc_in_host = ftsm_flash_write_buf(start_addr, buf, len, 1);
    if (ecc_in_host < 0 ) {
    ...
    /* ecc */
    ecc_in_tp = ftsm_fwupg_ecc_cal(start_addr, len);
    if (ecc_in_tp < 0 ) {
